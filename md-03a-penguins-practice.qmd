...
5.  What data types are contained in the variables? (Reminder: [https://rbtl-fs24.github.io/website/slides/lec-02-visualisation.html#/types-of-variables](https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables){.uri})
    -   species, island, sex: non-numerical (categorical variables)

    -    bill_length_mm, bill_depth_mm: numerical (continious)

    -   flipper_length_mm, body_mass_g, year: numerical (discrete)
...
```{r}
penguins |> 
  filter(species == "Gentoo", bill_length_mm >= 15.5)
```
...
```{r}
penguins |> 
  filter(species == "Chinstrap", sex == "female", island == "Dream" | island == "Biscoe")
```
...
-   NA values can be filtered out using the !is.na(flipper_length_mm) function
...
```{r}
penguins |> 
  filter(sex == "female", island == "Torgersen" | island == "Biscoe") |> 
  count(sex, island)
```
