...
```{r}
sanitation_spain_2000_2020 <- sanitation |> 
  filter(iso3 == "ESP",
         year  %in% c(2000,2020),
...
```{r}
ggplot(data = sanitation_spain_2000_2020,
       mapping = aes(
         x = residence,
         y = percent,
         fill= varname_long,
...
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?
    -   simplification of the data. Safely managed sanitation services is the highest category and the focus is on health indicators that need to be improved

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot?
    -   using the histogram makes a comparison between national, rural and urban easier
